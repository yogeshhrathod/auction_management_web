...
import { authInit } from "../authRedux/authAction";
import { initProfile } from "./../repository";
...
function Navbar(props) {
  ...
  const signOut = () => {
    localStorage.clear();
    authInit(props.dispatch, false);
  };

  const navPrivateLinks = [
    {
      name: "Post Auction",
      address: "/new-auction"
    },
  ...
  let links;
  if (props.auth) {
    links = navPrivateLinks;
  } else {
    links = navPublicLinks;
  }
  ...
  return (
    <div className="navbar-root">
      ...
      <div className={props.auth ? "show-profile" : "hide-profile"}>
        <div className="profile">
          ...
          <div className="dropdown-content">
            <Links links={links} isMobile={true} />
...
function Links(props) {
  const combineLinks = props.links.map((data, index) => (
    <Link
      key={index}
      to={data.address}
      className={props.isMobile ? "disp-on-mobile" : ""}
    >
      {data.name}
    </Link>
  ));
